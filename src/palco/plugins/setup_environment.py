"""Palco environment setup plugin."""
...
from palco.libraries.interactive_shell import get_interactive_console_options
...
def _is_async_hook_supported(hook_name: str, device_manager: DeviceManager) -> bool:
    ...
    bio_set = {
        device
        for device in device_manager.get_devices_by_type(PalcoDevice).values()
...
@hookimpl
async def palco_setup_env(
    config: PalcoConfig,
    cmdline_args: Namespace,
    plugin_manager: PluginManager,
    device_manager: DeviceManager,
) -> DeviceManager:
    """Palco environment setup for all the registered devices.
    ...
    hooks = (
        ...
        "palco_server_boot",
        "palco_server_configure",
        "palco_device_boot",
        "palco_device_configure",
        "palco_attached_device_boot",
        "palco_attached_device_configure",
    )
    for hook in hooks:
        await _run_hook(
            hook_name=hook,
...
def palco_post_setup_env(
    ...
    """Enter into palco interactive session after palco environment setup.
