"""Vitro interactive shell module."""
...
from vitro.libraries.utils import disable_logs
...
class OptionsTable:
    """Vitro interactive console options table."""
...
def _start_interactive_console(device: VitroDevice) -> None:
...
def _get_device_console_options(
    ...
    devices = device_manager.get_devices_by_type(VitroDevice).values()
...
def _interactive_ptpython_shell(
    ...
    if cmdline_args.legacy:
        devices = Namespace(**device_manager.get_devices_by_type(VitroDevice))
        rich_print(
...
def get_interactive_console_options(
    ...
    """Return options table with all vitro interactive shell options.

    :param device_manager: device manager instance
    :type device_manager: DeviceManager
    :param cmdline_args: command line arguments
    :type cmdline_args: Namespace
    :return: option table with vitro interactive shell options
    :rtype: OptionsTable
    """
    table = OptionsTable("VITRO INTERACTIVE SHELL")
    table.add_column("Choice", justify="center", style="cyan")
    ...
    if entry_points(group="pytest11", name="pytest_vitro"):
        table.add_option(
            ("e", "execute vitro automated test(s)"),
            _run_vitro_tests,
            (),
            {},
        )
