...
def _is_async_hook_supported(hook_name: str, device_manager: DeviceManager) -> bool:
    ...
    bio_set = {
        device
        for device in device_manager.get_devices_by_type(VitroDevice).values()
...
@hookimpl
async def vitro_setup_env(
    config: VitroConfig,
    cmdline_args: Namespace,
    plugin_manager: PluginManager,
    device_manager: DeviceManager,
) -> DeviceManager:
    """Vitro environment setup for all the registered devices.
    ...
    hooks = (
        ...
        "vitro_server_boot",
        "vitro_server_configure",
        "vitro_device_boot",
        "vitro_device_configure",
        "vitro_attached_device_boot",
        "vitro_attached_device_configure",
    )
    for hook in hooks:
        await _run_hook(
            hook_name=hook,
...
def vitro_post_setup_env(
    ...
    """Enter into vitro interactive session after vitro environment setup.
