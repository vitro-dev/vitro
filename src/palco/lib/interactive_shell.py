"""Palco interactive shell module."""
...
class OptionsTable:
    """Palco interactive console options table."""
...
def _get_device_console_options(
    ...
    devices = device_manager.get_devices_by_type(PalcoDevice).values()
...
def get_interactive_console_options(
    ...
    """Return options table with all palco interactive shell options.

    :param device_manager: device manager instance
    :type device_manager: DeviceManager
    :param cmdline_args: command line arguments
    :type cmdline_args: Namespace
    :return: option table with palco interactive shell options
    :rtype: OptionsTable
    """
    table = OptionsTable("PALCO INTERACTIVE SHELL")
    table.add_column("Choice", justify="center", style="cyan")
    ...
    if entry_points(group="pytest11", name="pytest_palco"):
        table.add_option(
            ("e", "execute palco automated test(s)"),
            _run_palco_tests,
            (),
            {},
        )
