...
from vitro.exceptions import DeviceNotFoundError, NotSupportedError
...
class DeviceManager:
    ...
    def register_device(self, device: VitroDevice) -> None:
        """Register a device as plugin with vitro.

        :param device: device instance to register
        :type device: VitroDevice
        """
        # During the registration of a plugin to the vitro, Pluggy calls all
        # of its properties. However, if a plugin has a method that throws an
        # exception, it can cause the vitro to crash. To address this issue
    ...
    def unregister_device(self, device_name: str) -> None:
        """Unregister a device from vitro.
...
def get_device_manager() -> DeviceManager:
    ...
    When you run vitro it will initialize the DeviceManager.
