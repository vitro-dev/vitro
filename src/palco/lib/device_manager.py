...
class DeviceManager:
    ...
    def register_device(self, device: PalcoDevice) -> None:
        ...
        :type device: PalcoDevice
        """
        # During the registration of a plugin to the palco, Pluggy calls all
        # of its properties. However, if a plugin has a method that throws an
        # exception, it can cause the palco to crash. To address this issue
    ...
    def unregister_device(self, device_name: str) -> None:
        """Unregister a device from palco.
...
def get_device_manager() -> DeviceManager:
    ...
    When you run palco it will initialize the DeviceManager.
